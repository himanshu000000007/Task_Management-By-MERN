...
import { BACK_BUTTON, DANGER_BTN, FULL_BUTTON, INPUT_WRAPPER, INPUTWRAPPER, personalFields, SECTION_WRAPPER, securityFields } from '../assets/dummy'
...
const Profile = ({setCurrentUser, onLogout }) => {
    ...
    const changePassword = async (e) => {
         ...
         try {
            const token = localStorage.getItem("token");
            const {data} = await axios.put(`${API_URL}/api/user/password`,{currentPassword: passwords.current, newPassword: passwords.new},
                {headers:{Authorization: `Bearer ${token}`}}
            )
            if(data.success){
    ...
    <div className='min-h-screen bg-gray-50'>
        ...
        <div className='max-w-4xl mx-auto p-6'>
            ...
            <div className='flex items-center gap-4 mb-8'>
                <div className='w-16 h-1/6 rounded-full bg-gradient-to-br from-fuchsia-500 to-purple-600 flex items-center justify-center text-white text-2xl font-bold shadow-md'>
                    ...
                    <form onSubmit={changePassword} className='space-y-4'>
                        {securityFields.map(({name, placeholder})=>(
                            <div key={name} className={INPUT_WRAPPER}>
                              ...
                              <input type="text" placeholder={placeholder} value={passwords[name]} 
                              onChange={(e)=> setPasswords({...passwords, [name]: e.target.value})} className='w-full focus:outline-none text-sm ' required/>
                            </div>
                        ))}
                        <button className={FULL_BUTTON}>
