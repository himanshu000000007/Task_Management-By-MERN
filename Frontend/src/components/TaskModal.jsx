...
import { AlignLeft, Calendar, PlusCircle, X, Save, CheckCircle, Flag } from 'lucide-react';
...
const TaskModal = ({ isOpen, onClose, taskToEdit, onSave, onLogout }) => {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        if (resp) {
          if (resp.status === 481) return onLogout?.();
  ...
  const baseControlClasses =
    'w-full border border-purple-200 rounded-lg px-3 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all duration-200';
      ...
      <div className="bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div
              className="flex items-center border border-purple-200 rounded-lg px-3 py-2.5 focus-within:ring-2 focus-within:ring-purple-500 transition-all duration-200"
            >
              <input
                type="text"
                name="title"
