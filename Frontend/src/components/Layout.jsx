...
const Layout = ({onLogout, user}) => {
 ...
 if(error) return (
    <div  className='min-h-screen bg-gray-50 p-6 flex items-center justify-center '>
        <div className='bg-red-50 p-4 text-red-600 rounded-xl border border-red-100 max-w-md'>
          <p className='font-medium mb-2'> Error loading tasks</p>
          <p className='text-sm '>{error}</p>
          <button onClick={fetchTasks} className='mt-4 py-2 px-4 bg-red-100 text-red-700 rounded-xl text-sm font-medium hover:bg-red-200 transition-colors'> Try Again!</button>
        </div>
    </div>
 )
    ...
    <div className= 'min-h-screen bg-gray-50'>
        ...
        <div className='ml-0 xl:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300'>
            <div className='grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6'>
               ...
               <div className='xl:col-span-1 spay4 sm:space-y-6 '>
                <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
                    ...
                    <div className='grid grid-cols-2 gap-3 sm: gap-4 mb-4 sm:mb-6'>
                      <StatCard title ='Total Tasks' value={stats.totalCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500'/>} />
                       <StatCard title ='Completed' value={stats.completedTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500'/>} />
                    <StatCard title ='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500'/>} />
                        ...
                        <div className=' relative pt-1'>
                            <div className='flex gap-1.5 items-center'>
                                   <div className=' flex-1 h-2 sm:h-3 bg-purple-100 rounded-full overflow-hidden'>
                                    <div className='h-full bg-gradient-to-r from-fuchsia-500 to-purple-600 transition-all duration-500' style={{width: `${stats.completionPercentage}`}}></div>
                                   </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='space-y-2 sm:space-y-3'>
                        {tasks.slice(0,3).map((task)=>(
                            <div key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100'>
                                ...
                                <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completed ? 'bg-gray-100 text-gray-700 ' : 'bg-fuchsia-100 text-fuchsia-700 '}`}>
